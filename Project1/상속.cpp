...
private:
    string recom;
public:
    Recbook(string name, string author, int price, int rating, string recom) :Book(name, author, price, rating),recom(recom){}
    void showInfo() {
        cout <<"평점"<< rating <<"점, " << author<<"의 "<< name <<endl;
        cout << recom << endl;
        cout << "판매가 " << price << "원" << endl;
    }
};
...
int main() 
{
    ...
    // Book으로 선언된 mybook 포인터인데 Recbook이 들어감!(다형성)
    cout << endl;
    mybook->showInfo();
    return 0;
}